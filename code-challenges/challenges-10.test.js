...
const createServer = () => {
  ...
  app.get('/hello', (req, res) => {
    res.send('Great day for hay');
  });
  app.get('/aboutme', (req, res) => {
    res.send('Pitter-patter, let’s get at ‘er');
  });
  app.get('/favoritefoods', (req, res) => {
    res.send(['Alberta Beef', 'Puppers', 'Gus n Brew']);
  });
...
const totalSum = (input) => {
  ...
  const holder = input.reduce((acc, val) => acc.concat(val), []);
  const sum = holder.reduce((acc, val) => acc + val, 0);
  return sum;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const output = data.reduce((acc, val, index) => {
    if (val.gender === 'male' || val.gender === 'female') {
      if (index > 1) {
        acc = acc + ` and ${val.name}`;
        //console.log(acc);
        return acc;
