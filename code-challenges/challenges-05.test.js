...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(person => {
    ...
    const $h2 = $newSection.find('h2');
    $h2.text(person.name);
    const $h3 = $newSection.find('h3');
    $h3.text(person.height);
    const $p = $newSection.find('p');
    $p.text(person.eye_color);
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  recipe.ingredients.forEach(modify => {
    const a = modify.indexOf(' ', 4);
    const slice = modify.slice(a + 1, modify.length);
    result.push(slice);
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(modify => {
    const split = modify.split(' ');
    const split1 = split[2];
    const split2 = split[3];
    //console.log(split[2]);
    if (split2 !== undefined) {
